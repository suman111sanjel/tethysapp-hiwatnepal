...
myApp.populateRiverSelect = function () {
    let htmlStr = ''
    NepalHIWATRivers.forEach(function (obj) {
        htmlStr = htmlStr + `<option value="${obj[0]}">${obj[1]}</option>`
    })
    $('#selRivers').html(htmlStr)
}
...
myApp.AddLayers = async function () {
    ...
    myApp.HiwatRiverWMSSource = new ol.source.TileWMS({
        url: datasource,
        params: {
            'LAYERS': 'HIWAT:rivernepal', 'TILED': true
        },
        serverType: 'geoserver',
        crossOrigin: 'anonymous'
    });
    ...
    myApp.HiwatRiver = new ol.layer.Tile({
        id: 'HIWATRivernepal',
        title: 'HIWAT Rivers Nepal',
        legendPath: LegendSource + 'HIWAT:rivernepal',
        visible: true,
...
myApp.UpdateExtentTo = function (curVal, DataArray) {
    let selAr = DataArray.filter(function (x) {
        return x[0] === curVal
    });
...
myApp.setSelForcastDateDropdown = function () {
    let totalNumberofBackDays = 7;
    let dateNew = new Date();
    dateNew.setDate(dateNew.getDate() - 1);
    myApp.DefaultForcastDate = myApp.formatDate(dateNew);
    let StrHtml = ''
    for (let a = 0; a < totalNumberofBackDays; a++) {
        let StrDate = myApp.formatDate(dateNew);
        StrHtml = StrHtml + `<option value="${StrDate}">${StrDate}</option>`
        dateNew.setDate(dateNew.getDate() - 1);
    }
    $('#selForcastDate').html(StrHtml)
    $('#selForcastDate').val(myApp.DefaultForcastDate);
}
myApp.ajaxCall = function () {
    let comid = myApp.Current_comid;
    let Date = $('#selForcastDate').val();
    $.ajax({
        type: "GET",
        data: {
            "stID": comid,
        ...
        "beforeSend": function (xhr, settings) {
            $('#ajax_loading_class').show();
            // $.ajaxSettings.beforeSend(xhr, settings);
        },
        "success": function (data) {
            $('#ajax_loading_class').hide()
            ...
            var layout1 = {
                title: 'Forecast at Date (Nepal standard time)' + json_response.rundate + ', River ID: ' + json_response.riverID,
                autosize: true,
                shapes: [{
                    type: 'rect',
                    xref: 'x',
        ...
        "error": function (data) {
            $('#ajax_loading_class').hide();
            let Date = $('#selForcastDate').val();
            alert("Data not available for " + Date);
...
myApp.BindControls = function () {
    $('#selDistrict').change(async function () {
        ...
        myApp.UpdateExtentTo(curVal, NewdistrictExtent);
    });
    $('#selRivers').change(async function () {
        let curVal = parseInt($(this).val());
        myApp.UpdateExtentTo(curVal, NepalHIWATRivers);
    });
    $('#selForcastDate').change(async function () {
        $('#graph').modal('hide');
        myApp.ajaxCall();
        $('#graph').modal('show');
    });
            ...
            if (url) {
                fetch(url)
                    ...
                    .then(function (html) {
                        ...
                        myApp.Current_comid = parsedJson.features[0].properties.comid;
                        $('#selForcastDate').val(myApp.DefaultForcastDate);
                        myApp.ajaxCall();
                    });
            }
...
myApp.init = function () {
    ...
    myApp.populateRiverSelect();
    myApp.setSelForcastDateDropdown();
     myApp.geocoding();
    myApp.BindControls();
    // myApp.LoadDefaults();
    // myApp.layerswitcher()
