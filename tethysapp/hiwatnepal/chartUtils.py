...
def getForecastData(comid,dateComplete):
    runDate = dt.datetime.now().date() - timedelta(1)
    formatDate = runDate.strftime('%Y%m%d')

    if formatDate == dateComplete:
        request_params = dict(comid=comid, cty='nepal')
    else:
        # request_params = dict(comid=comid, cty='nepal',completeDate=dateComplete)
        request_params = dict(comid=comid, cty='nepal'+str(dateComplete))
    request_headers = dict(Authorization=AuthorizationToken)
